...
-(void)updateServerFriendStatus:(BOOL)addedIsZeroHideIsOne withValue:(BOOL)value{
    ...
    dispatch_async(fetchQ, ^{
        ...
        if (!addedIsZeroHideIsOne) {
            str = [NSString stringWithFormat:@"http://busbookie.com/serverlets/addfriendtochatjson.php?session=%@&friendid=%@&add=%@&date=%@",sessionid, [(Friend *)self.annotation userID], valueString, dateString];
            
            // Hiding
        }else{
            str = [NSString stringWithFormat:@"http://busbookie.com/serverlets/hidefromfriendjson.php?session=%@&friendid=%@&hide=%@&date=%@",sessionid, [(Friend *)self.annotation userID],valueString, dateString];
        }
